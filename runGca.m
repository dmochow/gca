function [What,Vhat,G_gca,stats] = runGca(X,L,P,maxIters,maxcond)
%[WHAT,VHAT,G_GCA,STATS]=RUNGCA(X,L,P,MAXITERS,MAXCOND)
...
%   covariance matrices of the observations "this is a form of regularization
%       defaults to inf or no regularization 
%   
%   WHAT: D x P array of weights on DRIVING signals
%   VHAT: D X P array of weights on DRIVEN signals
%   G_gca: P x 1 strength of causality between driving and driven signals
%   stats: miscellaneous statistics including convergence
%
%   Jacek P. Dmochowski, 2022


optTol=1e-6; % internal stopping parameter

if nargin<5 || isempty(maxcond), maxcond=inf; end %  cov matrices with cond #s > maxcond will be regularized 
...
Xr=X(end:-1:1,:); % create reversed time

%% check whether data is time x dimension
if D>N
    warning('Number of dimensions exceeds the number of samples, please check the shape of input X')
end

%% initialize parameters for fmincon
vt=0.01*randn(D,1);
wt=0.01*randn(D,1);
...
for p=1:P
    ...
    % compute the spatiotemporal stats of the observations
    [blkR1,Rtilde] = gcBlockCovariance(X,L);
    [blkR1r,Rtilder] = gcBlockCovariance(Xr,L);
    
    % regularize covariance matrices if requested
